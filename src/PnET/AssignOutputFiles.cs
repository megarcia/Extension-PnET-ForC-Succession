...
namespace Landis.Extension.Succession.BiomassPnET
{
    ...
    {
        // Maps the output files by pixel that are prescribed by
        // the user with the term "PnETOutputSites", i.e. cohort
        // and location specific output files. The user can supply 
        // the location through the row and column number of the 
        // map location where they want to see output, or though 
        // coordinates X and Y. If using coordinates, the user 
        ...
        public struct ParameterNames
        {
            ...
            // This is used to get the parameter names that the 
            // user can supply; the model checks if the input 
            // parameters supplied by the user are known by the 
            // model. 
        ...
        /// </summary>
        /// <param name="InputMapLocation"></param>
        /// <param name="MapTopLeftCorner"></param>
        /// <returns></returns>
        ...
        public static void MapCells(Dictionary<string, Parameter<string>> outputfiles, ref Dictionary<ActiveSite, string> OutputSiteNames)
        {
            foreach (KeyValuePair<string, Parameter<string>> site in outputfiles)
            {
                ...
                if (site.Value.ContainsKey(ParameterNames.MapCoordinatesX) && site.Value.ContainsKey(ParameterNames.MapCoordinatesY))
                {
                    ...
                    if (Y >= MaxY)
                        throw new System.Exception("Cannot assign output location, Y coordinate " + Y + " should be larger than MaxY" + MaxY);
                    if (X >= MaxX)
                        throw new System.Exception("Cannot assign output location, X coordinate " + X + " should be larger than MaxY" + MaxX);
